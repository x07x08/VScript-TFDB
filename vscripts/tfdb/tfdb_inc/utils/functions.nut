...
{
	local bRedFound, bBluFound, hClient, iTeam;
	...
	for (local iIndex = 1; iIndex <= MAX_PLAYERS; iIndex++)
	{
		...
		iTeam = hClient.GetTeam();

		if (iTeam == TF_TEAM_RED)
		{
			if (bBluFound) return true;

			bRedFound = true;
		}
		else if (iTeam == TF_TEAM_BLUE)
		{
			if (bRedFound) return true;
...
local RAD2DEG = 57.295779513; // 180 / PI = 57.29577951308232087679

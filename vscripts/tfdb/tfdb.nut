...
{
	IncludeScript(strParentFolder + "tfdb_inc/" + strScript, ROOT);
}

TFDB.AlreadySetUp <- (("TFDB" in ROOT) && ("AlreadySetUp" in ROOT.TFDB)) ? true : false;
...
function TFDB::SelectTarget(iTeam, hRocketEntity = null)
{
	...
	for (local iIndex = 1; iIndex <= MAX_PLAYERS; iIndex++)
	{
		...
		fNewWeight = RandomFloat(0.0, 100.0);
...
function TFDB::EnableDodgeball()
{
	...
	try { IncludeScript(strParentFolder + "classes.nut", ROOT); } catch(e) {}
	try { IncludeScript(strParentFolder + GetMapName() + ".nut", ROOT); } catch(e) {}
...
try { IncludeScript(strParentFolder + "tfdb_custom/main.nut", ROOT); } catch(e) {}
try { IncludeScript(strParentFolder + "tfdb_custom/" + GetMapName() + ".nut", ROOT); } catch(e) {}
